# agent-knowledge-governance — 문서 JSON 스펙 (v0.5.0)
...
```jsonc
"body": {
  "owner": "TESTUSER",                      // 필수, 대문자
  "table": "FDC_SENSOR",                    // 필수, 대문자. id = lower(owner.table)
...
"body": {
  ...
  "argumentHint": "{snsr_id}",              // 필수 — 표시용 인자 힌트
...
[
  ...
  { "keywords": ["testuser.fdc_sensor", "fdc_sensor"],
    "path": "docs/testuser.fdc_sensor.md" },
  // ② pointer 묶음 — 뒷순서, precision 0.5(포인터 주입)
  { "keywords": ["sensor"],
    "path": "docs/testuser.fdc_sensor.md", "precision": 0.5 }
]
```
...
```jsonc
// store/db-schema/testuser.fdc_sensor.json
{
  "schema": "db-schema/v1",
  "id": "testuser.fdc_sensor",
  "keywords": [
...
`rendered/db-schema/docs/testuser.fdc_sensor.md`(SNSR_ID는 `{{설명}}`, EQP_ID는
